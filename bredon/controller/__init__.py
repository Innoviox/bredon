...
class MonteGame(TextGame):
    def __init__(self, size=5, board=None, white=HUMAN, black=Monte(10000)):
        super().__init__(size=size, board=board, white=white, black=black)
    ...
    def _init_player(self, color, types) -> Player:
        ...
        if name == HUMAN[0]:
            ...
            return MonteAI(self.board.size, color, time=depth)

    def _run(self, player, turn, input_fn=input):
        self.viz()
        t = time.time()
        if isinstance(player, MonteAI):
            m = player.pick()
            c = player.color
            if turn <= 1:
                c = c.flip()
    ...
    @property
    def monte(self):
        return [i for i in self.players if isinstance(i, MonteAI)][0]
